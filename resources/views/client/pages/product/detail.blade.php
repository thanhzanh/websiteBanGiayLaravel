    ...
    <div class="content max-w-7xl mx-auto p-6 bg-white shadow-lg rounded-lg">
        ...
        <div class="flex flex-col md:flex-row gap-8">
            <div class="md:w-1/2 flex relative mt-4 group">
                    ...
                    <div class="top-0 h-[460px]">
                        @foreach ($product->images->slice(1) as $index => $image)
                            <img class="w-full my-2 h-[140px] object-cover rounded-md shadow-md cursor-pointer transition-transform duration-300 ease-in-out hover:scale-105"
                                ...
                                data-index="{{ $index + 1 }}" title="{{ $product->product_name }}"
                ...
                <div class="w-[460px] h-[440px] right-2 mb-4 ml-4 overflow-hidden relative">
                    <button id="prevButton"
                        class="absolute left-0 z-10 text-black p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity top-1/2 transform -translate-y-1/2 text-3xl">
                        <i class="fa-solid fa-angles-left"></i>
                    </button>
                    @if ($product->images->isNotEmpty())
                        <img id="mainImage"
                            class="w-full h-auto rounded-lg shadow-lg object-cover transition-transform duration-300 ease-in-out transform hover:scale-125 hover:cursor-pointer"
                            src="{{ asset('storage/' . $product->images->first()->file_image_url) }}"
                            alt="{{ $product->product_name }}" title="{{ $product->product_name }}">
                    @endif
                    <button id="nextButton"
                        class="absolute right-0 z-10 text-black p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity top-1/2 transform -translate-y-1/2 text-3xl">
            ...
            <div class="md:w-1/2">
                ...
                <p class="text-gray-700 font-bold uppercase mt-3 text-[18px]"> Danh Mục :
                ...
                <form form-add-cart action="{{ route('cart.add', ['id' => $product->product_id]) }}" method="post">
                    ...
                    <div class="flex items-center justify-start mt-5">
                        ...
                        <button decrease-quantity
                            class="px-4 py-1 bg-gray-200 hover:bg-black border-gray-300 border hover:text-white">-</button>
                        <input quantity type="text" class="w-12 py-[4px] text-center border outline-none border-gray-300"
                            min="1" value="1">
                        <button increase-quantity
                            class="px-4 py-1 bg-gray-200 hover:bg-black border-gray-300 border hover:text-white">+</button>
                    </div>

                    <button type="button" btn-add-cart
        ...
        <div class="mt-8">
            <h2 class="text-[18px] font-bold text-gray-800 mb-4 pl-2">MÔ TẢ SẢN PHẨM</h2>
            <div
                class="bg-gray-50 p-6 rounded-lg shadow-md border border-gray-200 leading-relaxed text-gray-700">
